...
def train_resnet_pipeline(
    ...
    output_path: str = "model/bp_model_resnet",
    ...
    print("\n[Step 4/5] Training ResNet1D model (with Huber Loss)...")
    print(f"  Epochs: {epochs}, Batch size: {batch_size}")
    print(f"  Architecture: ResNet1D (18-layer equivalent) with SE Blocks\n")

    # Instantiate specific model and loss
    model = ResNet1D()
...
def _print_metrics(metrics: dict) -> None:
    ...
    print("\n" + "=" * 70)
    print("CNN/ResNet TRAINING RESULTS")
    print("=" * 70)
