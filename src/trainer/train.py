...
# Add project root to path (3 levels up from this file: src/trainer/train.py -> src/trainer/ -> src/ -> root)
project_root = Path(__file__).resolve().parent.parent.parent
sys.path.insert(0, str(project_root))

from src.trainer.resnet import train_resnet_pipeline
...
def validate_dataset_parts(parts):
    ...
    """
    base_path = project_root / "uci_dataset"
    missing_files = []
    valid_paths = []
...
Examples:
  ...
  python train.py --output models/my_custom_model.pkl
    ...
    try:
        ...
        metrics = train_resnet_pipeline(
            data_path=data_paths,
            output_path=args.output.replace('.pkl', '_resnet'),
